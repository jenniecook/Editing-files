...
class cmb_Meta_Box {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', "musicflow_theme" ),
					'defaultString' => __( 'Default', "musicflow_theme" ),
					'pick'          => __( 'Select Color', "musicflow_theme" ),
					'current'       => __( 'Current Color', "musicflow_theme" ),
				) );
			}
		} else {
	...
	public static function render_group( $args ) {
		...
		echo '<tr><td colspan="2"><table id="', esc_attr($field_group->id()), '_repeat" class="repeatable-group'. esc_attr($sortable) .'" style="width:100%;">';
		if ( $desc || $label ) {
			echo '<tr><th>';
				if ( $label )
		...
		echo '<tr><td><p class="add-row"><button data-selector="', esc_attr($field_group->id()) ,'_repeat" data-grouptitle="', esc_attr($field_group->options( 'group_title' )) ,'" class="add-group-row button">'. esc_html($field_group->options( 'add_button' )) .'</button></p></td></tr>';
	...
	public static function render_group_row( $field_group, $remove_disabled ) {
		...
		echo '
		<tr class="repeatable-grouping" data-iterator="'. esc_attr($field_group->count()) .'">
			<td>
				...
				if ( $field_group->options( 'group_title' ) ) {
					...
					<tr class="cmb-group-title">
						<th colspan="2">
							', sprintf( '<h4>%1$s</h4>', esc_html($field_group->replace_hash( $field_group->options( 'group_title' ) )) ), '
						<th>
					</tr>
					';
				}
				...
				echo '
					<tr>
						<td class="remove-row" colspan="2">
							<button '. esc_attr($remove_disabled) .'data-selector="'. esc_attr($field_group->id()) .'_repeat" class="button remove-group-row alignright">'. esc_html($field_group->options( 'remove_button' )) .'</button>
						</td>
					</tr>
				</table>
			</td>
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', "musicflow_theme") );

	if ( $echo )
		printf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', "musicflow_theme") );

	return sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', "musicflow_theme") );
}
